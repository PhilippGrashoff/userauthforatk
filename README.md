...
As I split the code of my Application into several "data" and "ui" repositories, this way would require to widely use Atk4\Ui\Apo in data layer.
...
A basic User model comes with this repo. However, Auth can be used with any User class as its independent of the User class.
...
In the current implementation, Auth only calls hooks in the user Model, so Auth does not need any knowlegde of the User class. 
Hence, User class can be easily exchanged.

### No usage of atk Session handling
At the moment, this repo does not use atk session handling, as I only needed the minimal functionality of storing the logged-in User in the Session. However, the normal Atk Session Handler could be easily used.
...
you will have to create a new Auth class which extends Auth, just overwriting the `$sessionKeyForUser` property.
